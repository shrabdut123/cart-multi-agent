...
class LocaleProductTag(BaseModel):
    """
    Defines locale-specific tags that can be associated with an item. For example, a tag of type `SLEEP_ON_IT_DAYS` with values.
    """ # noqa: E501
    name: StrictStr
    values: Optional[List[LocaleProductTagValue]] = None
    __properties: ClassVar[List[str]] = ["name", "values"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SLEEP_ON_IT_DAYS']):
            raise ValueError("must be one of enum values ('SLEEP_ON_IT_DAYS')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "values": [LocaleProductTagValue.from_dict(_item) for _item in obj["values"]] if obj.get("values") is not None else None
        })
        return _obj
