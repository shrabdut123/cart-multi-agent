...
def display_state(
    ...
    try:
        ...
        # Handle CART details
        cart_details = session.state.get("cart_details", [])
        if cart_details and any(cart_details):
            print("🛒 Cart Details:")
            for item in cart_details:
                if isinstance(item, dict):
                    name = item.get("name", "Unknown")
                    sku = item.get("sku", "Unknown SKU")
                    service_eligible = item.get("service_eligible", "Unknown price")
                    services = item.get("services", [])
                    print(f"  - {name} (sku: {sku}, service_eligible: {service_eligible}, services: {services})")
                elif isinstance(item, str):
                    print(f"  - {item}")
                else:
                    print(f"  - {item} (unrecognized format)")
        else:
            print("🛒 Cart Details: None")
        ...
        other_keys = [
            ...
            if k not in ["user_name", "cart_details", "interaction_history"]
        ]
        if other_keys:
